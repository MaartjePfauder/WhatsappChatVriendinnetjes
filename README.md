...
- number of tokens: 275488 (token counts are based on spaCy tokenization and are intended as an approximate measure)
...
- Message types:
  ...
  - WhatsApp placeholders such as "<Media weggelaten>" (media omitted) were excluded from the final `Doc` column.
...
- **Removing unwanted messages**:
  ...
  - Messages written by temporary member 'Camille Werk' were removed.
...
- `notebooks/Stats_chat_analysis.ipynb`  
  Jupyter Notebook with descriptive statistics and visualisations based on `corpus.csv`.
...
- **Doc** 
  The message after NLP processing with spaCy (stored as a string representation of the spaCy Doc object)
...
## 8. Statistical analysis and plots

In addition to corpus creation, a second notebook (`Stats_chat_analysis.ipynb`) performs descriptive statistical analysis of the chat data.

This notebook analyses:
- number of messages per participant
- average message length (in tokens) per participant
- message activity over time (monthly)
- most common words overall and per participant (after stopword removal)
- average part-of-speech usage per participant

All visualisations generated in this notebook are saved to the `plots/` folder for reuse outside the notebook.

### 8.1. Saved plots

The following plots are automatically saved when running `Stats_chat_analysis.ipynb`:

- `messages_per_person.png`  
  Number of messages sent by each participant.

- `avg_tokens_per_person.png`  
  Average number of tokens per message per participant.

- `messages_per_month.png`  
  Total message activity per month over the full time span.

- `messages_per_month_by_person.png`  
  Monthly message activity split by participant (stacked area plot).

- `most_common_words_overall.png`  
  Most frequent words across the entire chat corpus (stopwords removed).

- `most_common_words_<person>.png`  
  Most frequent words per participant.

- `average_pos_use_by_person.png`  
  Average part-of-speech usage per participant.

All plots can be regenerated by rerunning `Stats_chat_analysis.ipynb` from start to finish.
...
- Checked for missing values in key columns (`Doc`, `Tokens`, `Lemmas`, `Parts-of-speech`).
